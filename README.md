...
<body>
  <div class="app">
    ...
    <div class="servers">
      ...
      <div class="srvBottom">
        <button class="btnIcon" id="openSettings" title="Settings">⚙️</button>
      </div>
    </div>
      ...
      <div class="chanTop">
        <div class="brand">
          ...
          <div class="brandText">
            ...
            <div class="t2" id="healthLine">Checking Ollama…</div>
          </div>
        </div>
        <button class="btnIcon" id="refreshAll" title="Refresh">↻</button>
      </div>

      <div class="chanScroll">
        <div class="sectionTitle">
          <div>Text Channels</div>
          <button class="btnIcon" id="newChannel" title="Create Channel">＋</button>
        </div>
      ...
      <div class="topbar">
        <div class="topLeft">
          <div class="chanTitle">
            ...
            <span class="name" id="activeTitle">—</span>
          </div>
        ...
        <div class="actions">
          ...
          <button class="btnIcon" id="btnPins" title="Pins">📌</button>
          <button class="btnIcon" id="btnMembers" title="Toggle Members">👥</button>
        </div>
      </div>

      <div class="chatScroll" id="chat">
        <div class="welcome" id="welcomeBox">
          Welcome to <b>#general</b> — this chat behaves like a Discord channel.<br/>
      ...
      <div class="composer">
        <div class="replyBar" id="replyBar">
          <div class="row" style="gap:10px; width:100%;">
            ...
            <button class="x" id="cancelReply" title="Cancel reply">✕</button>
          </div>
        </div>
        ...
        <div class="composerRow">
          <button class="btnIcon" id="btnEmoji" title="Quick reactions">😀</button>
    ...
    <div class="modal">
      ...
      <div class="modalBody">
        ...
        <div class="card" style="margin-top:12px;">
          ...
          <div class="tiny" style="margin-top:8px;">
            • Reply by hovering a message and clicking ↩︎ (if enabled)<br/>
            • React by hovering a message and clicking 😀 (if enabled)<br/>
            • Search with the box in the top bar (Enter)<br/>
            • Pins show in the right panel (if enabled)
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="modalBack" id="emojiBack">
    <div class="modal" style="width:min(520px, 100%);">
      <div class="modalHead">
        ...
        <button class="closeX" id="closeEmoji">✕</button>
      </div>
      <div class="modalBody">
...
`);
    ...
    async function renderRightPanel(){
      ...
      if (activeRightTab === "pins"){
        ...
        body.innerHTML = `<div class="tiny">Loading pins…</div>`;
        try{
    ...
    async function loadHealth(){
      try{
        ...
        el("healthLine").innerHTML =
          `Ollama: <b>${oll}</b> • OpenRouter: <b>${or}</b> • Puter: <b>${pr}</b> • Default: <b>${def}</b>`;
      } catch {
    ...
    function renderReplyPreview(m){
      ...
      prev.innerHTML = `Replying to <b>${escapeHtml(authorName(target))}</b>: ${escapeHtml((target.content||"").slice(0,120))}${(target.content||"").length>120?"…":""}`;
      return prev;
    }
    ...
    function renderTypingLine(){
      ...
      el("typingLine").textContent = names.length ? `${names.join(", ")} typing…` : "";
    }
    ...
    function startReply(messageId){
      ...
      if (!m) return;
      el("replyText").innerHTML = `Replying to <b>${escapeHtml(authorName(m))}</b>: ${escapeHtml((m.content||"").slice(0,120))}${(m.content||"").length>120?"…":""}`;
    ...
    function addDevEvent(evt){
      ...
      if (stage === "gate_check_start"){
        ...
        title = respond ? "Gate: ✅ will respond" : "Gate: ❌ skip";
        lines.push(`Character: ${who || "—"}`);
        if (provider) lines.push(`Gate backend: ${provider}`);
        if (d.gate_model) lines.push(`Gate model: ${d.gate_model}`);
        if (d.decision) lines.push(`Decision: ${d.decision}`);
        if (d.yes_count != null || d.no_count != null) lines.push(`Votes: YES ${d.yes_count ?? 0} • NO ${d.no_count ?? 0}`);
      ...
      wrap.innerHTML = `
        <div style="display:flex; justify-content:space-between; gap:10px; align-items:flex-start;">
          ...
          <div class="tiny">${escapeHtml(who ? who + " • " : "")}${escapeHtml(when)}</div>
        </div>
    ...
    async function setActiveConversation(convId){
      ...
      const conv = conversationsById[convId];
      el("activeTitle").textContent = conv ? conv.title : "—";
    ...
    async function runSearch(){
      ...
      el("rightBody").innerHTML = `<div class="tiny">Searching for <b>${escapeHtml(q)}</b>…</div>`;
      try{
        ...
        results.reverse().forEach(m => {
          ...
          div.innerHTML = `
            ...
            <div class="tiny" style="margin-top:8px; white-space:pre-wrap">${escapeHtml((m.content||"").slice(0,220))}${(m.content||"").length>220?"…":""}</div>
            <div style="margin-top:10px;">
    ...
    el("btnEmoji").onclick = () => {
      ...
      if (!replyToId){
        alert("Tip: hover a message and click 😀 to react. Or reply first to select a target.");
        return;
      }
